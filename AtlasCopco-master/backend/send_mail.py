...
def app():
    ...
    for r in range(2, sheet.max_row + 1):
        ...
        if (designation == 'Employee' and
                (taskStatus == 'DELAYED' or taskStatus == 'INPROCESS')):
    ...
    for manager_email, employees_list in managers_and_employees.items():
        ...
        # Send the email using smtplib
        sendmailStatus = smtpObj.sendmail(
            'shreyag0104@outlook.com', manager_email, msg.as_string())
        print(f'Sending email to {manager_email}...')
        if sendmailStatus != {}:
            print('There was a problem sending email to %s: %s' %
                  (manager_email, sendmailStatus))
